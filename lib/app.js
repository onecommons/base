...
function getConfigLoader(options) {
  ...
  global.configPaths = [path.resolve(options.configdir), path.join(__dirname, '..')];
...
AppView.prototype.render = function(options, fn){
  ...
  options.df = dataform.dataform({
    // default to GMT
    // (note: use moment().format('Z') to get local tz (e.g. returns '-08:00'))
    tz: options.displayTz || this.app.config.displayTz || 'Z'
  });
