...
function getConfigLoader(options) {
  ...
  return configloader(options.configOverrides,
  ...
  function guard(req, res, next) {
    if (req.user || req.path.match(app.config.privatemode.whitelist || /(login)|(auth)/)) {
      ...
      if (app.config.privatemode.message) {
        req.flash('success', app.config.privatemode.message);
      }
...
configRequestLogging: function() {
  var app = this;
  var config = app.config;
  if (typeof config.request_logger == 'function') {
    config.request_logger(app);
  } else if (typeof config.request_logger == 'string') {
    //'morgan' or 'logger' or 'none'
    if (config.request_logger !== 'none')
      log.requestLoggers[config.request_logger](app);
  } else {
    //default to morgan
    var type = (config.request_logger && config.request_logger.type) || 'morgan';
    log.requestLoggers[type](app, config.request_logger);
  }
}
...
function createApp(root, options) {
  ...
  app.use(log.useReqLog(config.request_logging && config.request_logging.childopt));
