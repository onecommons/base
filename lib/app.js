...
function createApp(root, options) {
  ...
  var app = express();
  var baseapp = createBaseApp();
  if (options.views) {
    app.set('views', options.views);
  }
  baseapp.use(app);
  //delegate app methods to the parent app
  Object.keys(appmethods).forEach( function(name) {
    app[name] = function() {
      return baseapp[name].apply(baseapp, arguments);
    };
  });
  //except for these methods:
  app.updateNamedRoutes = appmethods.updateNamedRoutes;
  app.start = function() {
    ...
    return baseapp.start.apply(baseapp, arguments)
  }
  return app; //base app is available through app.parent
}
