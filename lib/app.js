...
AppView.prototype.lookup = function(locate) {
  ...
  //note: this logic isn't important since views are internally specified
  // var ext = this.ext;
  // // <path>.<engine>
  // var resolved = utils.searchPath(this.root, locate);
  // if (resolved)
  //   return resolved;
  //
  // // <path>/index.<engine>
  // locate = path.join(path.dirname(locate), path.basename(locate, ext), 'index' + ext);
  // resolved = utils.searchPath(this.root, locate);
  // if (resolved)
  //   return resolved;
}
...
updateNamedRoutes: function(routes) {
  var namedRoutes = this.get("namedRoutes") || {};
  namedroutes.update(this, namedRoutes, routes);
  return namedRoutes;
},
...
start: function(ready, closeCallback) {
  var app = this;
  namedroutes.apply(app.get("namedRoutes"));
...
function createApp(root, options) {
  ...
  require('./swigextensions')(swig, {
   loaderPath: viewpath,
   cache: config.cacheviews !== undefined ? config.cacheviews : options.cacheviews,
  });
  ...
  app.set("namedRoutes", app.updateNamedRoutes(routes(app, passport)));
