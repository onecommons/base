...
/*
Returns a function that can be used with promises and Node-style callbacks
*/
makeErrorResponse: function(res, statusCode) {
  //if ('development' == this.get('env')) { return function(); }
  return function(err) {
    //res.redirect('/error/' + (statusCode||500));
...
function brequire(module) {
  var basepath = process.env.BASESRC ? path.resolve(process.env.BASESRC)
                  : path.join(__dirname, '..');
  if (module.charAt(0) == '.')
