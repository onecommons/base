...
MongoRawDatastore.prototype = {
  ...
  */
  ,create: function(mjson, cb) {
    ...
    this.collection.insert(mjson, this.defaultOptions, function (err, result) {
      //newer versions of mongo return ops array instead of doc array
      var doc = result && (result.ops ? result.ops[0] : result[0]);
      if (cb) {
        cb(err, doc);
      }
    });
  ...
  , prepare: function(mjson, promises) {
    //do nothing, let requests execute in parallel
    return Promise.resolve(mjson);
  }
};
...
MongooseDatastore.prototype = {
    ...
    this._update(mjson, cb, function(mjson, obj) {
      for (var key in mjson) {
        ...
        if (currentVal === undefined) {
          ...
          (valIsArray ? val : [val]).forEach(function(item) {
            obj._doc[key].push( item );
          });
        }
      }
      return true;
  ...
  ,update: function(mjson, cb, principle) {
     this._update(mjson, cb, function(mjson, obj) {
      for (var key in mjson) {
        ...
        // note: this won't merge if the key isn't in the schema
        obj.set(key, mjson[key], null, {merge: true});
      }
      return true;
    }, principle);
  }
 ...
 , prepare: function(mjson_, promises) {
   // serialize operations on the same document
   // prior operations may update version counter, make sure this operation has the
   // latest or it might fail
  var jo = _getmjson2(mjson_);
  var mjson = jo[0], options = jo[1];
  var dependant = null;
  if (mjson._id) {
    //find the last one we depend on
    promises.forEach(function(p) {
      if (p.request.params._id == mjson._id) {
        dependant = p;
      }
    });
  }
  if (dependant) {
    return dependant.then(function(result) {
      if (result.__v !== undefined && mjson.__v !== undefined) {
        mjson.__v = result.__v;
      }
      return [mjson, options];
    });
  } else {
    return Promise.resolve([mjson, options]);
  }
}
...
RequestHandler.prototype.add = function(mjson, respond, p, session) {
    var This = this;
    this.datastore.prepare(mjson, p).then(function(mjson) {
     ...
     respond(This._getJsonrpcResponse(err));
   });
}
