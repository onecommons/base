          ...
          try {
            ...
            if (typeof ret !== 'undefined') {
              if (typeof ret.then === 'function') {//it's a promise
                var sendResponse = function(result) {return makeResult(req, result)};
                var sendErrorResponse = function(result) {
                  if (session.httpRequest && session.httpRequest.log)
                    session.httpRequest.log.warning("jsonrpc method call was rejected", result, req.method);
  ...
  var p = Promise.all(setup(body, methods, session)).then(
   function (result) {
     ...
     result = result.filter(function(elem){
       var keep = !!elem.id;
       if (elem.error && req.log) {
          req.log.warn(elem.error, "error when invoking jsonrpc method");
       }
       return keep;
     });
     if (!result.length) {
