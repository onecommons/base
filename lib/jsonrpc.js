...
function makeResult(request, responseObj) {
  ...
  if (responseObj instanceof JsonRpcError) {
    ...
    if (responseObj.data && !httpRequest.hideInternals) {
      response.error.data = responseObj.data;
    }
    if (httpRequest.id) {
...
function handleRequest(body, methods, req, res, session) {
  ...
  var p = Promise.all(setup(reqs, methods, session)).then(
   function (result) {
     ...
     result = result.filter(function(elem, index){
       var keep = !!elem.id;
       if (req.log) {
          var rpcreq = reqs[index];
          var logdata = { jsonrpccall: {method: rpcreq.method, params: rpcreq.params}};
          if (elem.error) {
            logdata.jsonrpccall.error = elem.error;
            req.log.warn(logdata, "error when invoking jsonrpc method");
          } else {
            logdata.jsonrpccall.result = elem.result;
            req.log.info(logdata, 'jsonrpc call');
          }
       }
       return keep;
     });
