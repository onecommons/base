...
var truncate = require('./truncate')({MAXDEPTH:5});
...
function makeResult(request, responseObj) {
  ...
  if (responseObj instanceof JsonRpcError) {
    ...
    if (httpRequest.log) {
      httpRequest.log.warn(truncate(logdata), "error when invoking jsonrpc method");
    }
    if (httpRequest.id) {
    ...
    if (httpRequest.log) {
      ...
      httpRequest.log.info(truncate(logdata), 'jsonrpc success');
    }
  }
  return response;
}
