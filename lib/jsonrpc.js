        ...
        p = new Promise(function (resolve, reject) {
          try {
            ...
            if (typeof ret !== 'undefined') {
              ...
              if (req.log)
                req.log.error(e, "unexpected error from jsonrpc method call:", req.method, "params:", req.params);
  ...
  var p = Promise.all(setup(body, methods, session)).then(
   function (result) {
     ...
     result = result.filter(function(elem){
       var keep = !!elem.id;
       if (!keep && elem.error) {//warn so these error aren't silently dropped
         if (req.log)
           req.log.error(elem.error, "error when invoking jsonrpc notification");
       }
       return keep;
     });
     if (!result.length) {
