...
module.exports.useReqLog = function(hideInternals) {
  return function(req, res, next) {
    var logger = req.app.log;
    // if you want an alternate id, use middleware to set it before this:
    if (typeof req.id === 'undefined')
      req.id = uuid.v4();
    var opt = { req_id: req.id };
    res.locals.req_id = req.id;

    // simple child loggers are much faster to instantiate
    var childLogger = logger.child(opt, true);
    req.log = childLogger;

    if (hideInternals)
      req.hideInternals = hideInternals;
    return next()
  };
}
...
module.exports.requestLoggers = {
  ...
  logger: function(app, options) {
    app.use( configureRequestLogging(options) );
  }
}
...
*/
function configureRequestLogging(options) {
  var immediate_level = options.immediate_level || 'debug';
  ...
  return function logRequest(req, res, next) {
    ...
    if (headers)
      requestInfo.req.headers = req.headers;
    //if (body)
    //  body: req.body XXX should handle max size
    childLogger[immediate_level](requestInfo, "%s %s", req.method, req.url);
    ...
    function logging() {
      ...
      var responseTimeMs = hrtime[0] * 1e3 + hrtime[1] / 1e6;
      var responseInfo = {
        method: req.method,
        url: req.originalUrl || req.url,
        sessionID: req.sessionID,
