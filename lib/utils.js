...
function requirePermission(permission, req, res, next) {
  ...
  if (!ok) {
    if (!req.user) {
      return isLoggedIn(false, req, res, next);
    }
    res.statusCode = 403;
    return next(new Error("Permission Denied. You don't have sufficient rights to access this page."));
  }
  return next();
}
...
exports.getPathValue= function(obj, propertyPath) {
  var target = obj;
