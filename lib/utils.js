...
function ensureAccess(guard, req, res, next) {
  var user = req.user;//XXX || guestuser;
  if (!user || !access.check(user, req, access.ruleFactory()(guard) )) {
...
// route middleware to make sure a user is logged in
exports.ensureAccess = function(guard) { return _.partial(ensureAccess, guard); }
