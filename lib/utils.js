...
var isLoggedIn = function(doRecentCheck, req, res, next) {
  ...
  if (failed) {
    ...
    req.flash('login.danger', req.session.reauthenticate ? 'You need to login again to access this page.'
  ...
  if (config.requireEmailVerification
    ...
    if (config.requireEmailVerification == 'nag') {
      req.flash('warning', 'Please confirm your email address by clicking the link sent to <strong>'+ req.user.local.email +"</strong>&nbsp;<a class='send-resend' href='#'>Resend email</a>.");
