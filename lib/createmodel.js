...
var createSchema = function(modelName, schema, optionsOrbaseModel, accesscontrol) {
  ...
  if (baseSchema) {
    ...
    if (!isSchemaInstance) {
      ...
      if (!options.toJSON) {
        options.toJSON = { getters: true };
      }
      if (options.emitIndexErrors === undefined) {
        options.emitIndexErrors = true
      }
      ...
      if (schema.get('emitIndexErrors') === undefined) {
        schema.set('emitIndexErrors', true)
      }
    }
    ...
    schema.pre('save', function(next){
       try {
         if(this.isNew && !this._id){
          ...
          //this will fail with embedded docs but we always want an id, so fallback:
    ...
    schema.virtual('createdOn').get(function() {
      if (!this.id)
        return null;
      return extractTimestamp(this.id);
    });
    //model = mongoose.model(modelName, schema);
 }
 if (options && options.ui) {
   // ignored by schema ctor so need to apply this after the schema is created
   schema.ui = options.ui;
 }
 if (schema.ui && schema.ui.titlefields) {
   var titlefields = schema.ui.titlefields.split(' ');
   schema.virtual('title').get(function() {
     var This = this;
     return titlefields.map(function(path) {return This.get(path)||''}).join(' ');
   });
 }
 schema.metadata = {
    modelName: modelName,
...
createModel.extractTimestamp = extractTimestamp;
