...
module.exports.dataform = function(){
   ...
   df.formhandler = function() {
      return "$(document).on('submit', '.dbform', function() { $(this).dbUpdate(); return false; });\n"
   }
       ...
       for(var i = 0; i < options.length; i++){
           ...
           if (typeof options[i] !== 'object') {
             ...
             value = pyPop(htmlattr,
               htmlattr.hasOwnProperty(_IDkey) ? _IDkey : valuekey);
   ...
   df.select = function(prop, options, htmlAttrs){
       ...
       console.log('selection', selection, misc.trueTypeof(selection));
