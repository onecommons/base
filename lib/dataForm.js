...
module.exports.dataform = function(context){
  ...
  df.getVal = function(prop) {
   ...
   df.textarea = function(prop, htmlAttrs){
       var obj = getObj();
       var value = resolve(obj, prop, pyPop('defaultValue'));
       setFormBindClass(value, htmlAttrs);
   ...
   function generateOptions(options, selection, valuekey, labelkey, forceValue){
       var rv = '';
   ...
   df.select = function(prop, options, htmlAttrs){
       ...
       var selectedFunc = function(value) {
         if (selection === undefined || selection === null) {
           ...
           return selection[objectValueKey] == value;
         } else {
       ...
       rv +=  generateOptions(optionArray,selectedFunc, valuekey, labelkey, forceValue);
       rv += '</select>';
       return rv;
   }
   df.select.safe = true;
