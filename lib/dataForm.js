...
module.exports.dataform = function(context){
  ...
  df.getVal = function(prop) {
   ...
   df.select = function(prop, options, htmlAttrs){
       ...
       rv += optionInfo.rv;
       if (!optionInfo.selectionFound) {
         // the value not found in select list, add it and set as selection
         var optionAttr = {};
         setFormBindClass(selection, optionAttr);
         if (forceValue) {
          optionAttr.value = serializeValue(selection);
         }
         rv += '<option selected ' + kwtoattr(optionAttr) + '>';
         rv += serializeValue(selection).replace(/&/g,'&amp;').replace(/</g,'&lt;');
         rv += '</option>\n';
       }
       rv += '</select>';
       return rv;
