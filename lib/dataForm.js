...
module.exports.dataform = function(){
   ...
   function setFormBindClass(val, html_attrs, nullAsJSON){
       var btype, bindClass;
      if(trueTypeof(val) === 'string') {
              return '';  // normal string: don't bother with string.

      } else if(trueTypeof(val) === 'Array') {
          btype = 'json';

      } else if(trueTypeof(val) === 'boolean'){
          btype = 'boolean'

      } else if(trueTypeof(val) === 'number'){
          btype = 'number'

      } else if(trueTypeof(val) === 'null'){
   ...
   df.select = function(prop, options, htmlAttrs){
       ...
       var isArray = trueTypeof(selection) === 'Array';
       if(isArray) {
           prop += '[]';
       }
