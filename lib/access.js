...
Policy.prototype = {
  ...
  lookupOperation: function(op) {
    return typeof op === 'string' ? this._operations[op] : op;
  },

  findRules: function(op, accessMap) {
    var opqual = op.split(':');
    op = opqual[0]
    var qualifier = opqual[1] ? ':'+opqual[1] : '';
    var obj = this.lookupOperation(op);
...
function createChecker(map, policy) {
  ...
  return {
    ...
    getRules: function(op) {
      return policy.findRules(op, normalizedMap);
    },
