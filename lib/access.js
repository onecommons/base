...
*/
function ruleFactory(policy) {
  var s = policy ? policy : new Policy();

  function _getGuards(spec) {
    if (Array.isArray(spec)) {
      if (spec.length < 1)
        throw new InvalidAccessSpec(spec, "empty guard array");
      return spec.map(s.lookupPermission, s);
    } else {
      var guard = s.lookupPermission(spec);
      if (guard !== undefined) {
        return [guard];
      } else {
        return undefined;
...
function _getAndCheckGuards(principle, object, spec, rel) {
  ...
  function _checkGuards(principle, object, guards, rel) {
    if (guards.indexOf(s.emptyPermission) != -1)
      return true;
    return s.hasPermission(principle, object, rel, guards);
  }
...
Policy.prototype = {
  ...
  defaultPrinciple: null,

  lookupPermission: function(permission) { //permissions are just strings
    return typeof permission === 'string' ? permission : undefined;
  },

  hasPermission: function(user, object, rel, guards) {
  ...
  lookupOperation: function(op, ops) {
    return typeof op === 'string' ? ((ops?ops[op]:null) || this._operations[op]) : op;
  },

  findRules: function(op, accessMap, ops) {
    var opqual = op.split(':');
    op = opqual[0];
    var qualifier = opqual[1] ? ':'+opqual[1] : '';
    var obj = this.lookupOperation(op, ops);
    if (!obj) throw new InvalidAccessSpec(null, "unknown op: " + op);
    var rules = [];
    //console.log('a', obj, accessMap);
    while (obj) {
      var rule = accessMap[obj.name + qualifier];
      if (rule)
        rules.push(rule);
      obj = obj.base;
    }
...
function createPermissionsChecker(policy) {
  policy = policy || new Policy();
  var makeRule = ruleFactory(policy);
  return {
    policy: policy,
    check: function(user, obj, permission) {
      return check(user, obj, makeRule(permission))
    },
    ensure: function(user, obj, permission) {
      return ensure(user, obj, makeRule(permission))
    }
  };
}

function createOperationChecker(map, policy) {
  var maps = Array.isArray(map) ? map : [map];
  policy = policy || new Policy();
  var makeRule = ruleFactory(policy);
  var moreOps = _.compact(_.pluck(maps, '__operations'));
  moreOps = makeOps(_.defaults.apply(_, moreOps) || {});
  ...
  return {
    ...
    getRules: function(op) {
      return policy.findRules(op, normalizedMap, moreOps);
    },
...
InvalidAccessSpec.prototype = new Error;
...
exports.createOperationChecker = createOperationChecker;
//internals:
exports.makeMap = makeMap;
exports.ruleFactory = ruleFactory;
exports.createPermissionsChecker = createPermissionsChecker;
