...
module.exports = function(passport, app) {
    ...
    function(req, email, password, done) { // callback with email and password from our form
        ...
        User.findOne({ 'local.email' :  email }, function(err, user) {
            ...
            if (!user)
                return done(null, false, req.flash('login.danger', 'Oops! Wrong email or password')); // req.flash is the way to set flashdata using connect-flash

            if (config.requireEmailVerification && !user.local.verified) {
                auth.recordLogin(user, 'unverified', req.ip);
                return done(null, false,
                  req.flash('login.warning', 'This account has not been verified. Please check your email. <a href="/verification-resend">Resend email</a>'));
            }
