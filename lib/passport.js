            ...
            if (!user.validPassword(password)) {
               ...
               //XXX should there be a config option to make lockout permanent? (e.g. contact us to re-enable)
                ...
                if (user.local.failedLoginAttempts >= config.failedLoginAttempts) {
                  ...
                  log.warn("account %s has been locked for %s", user.id, lockTimeDescription);
                ...
                user.save(function(err) {
                    if (err) {
                        log.error(err, "error saving user %s with invalid password", user.id);
                        return done(err);
                    }

                    //updating user with failed login counts
                ...
                user.save(function(err) {
                    if (err) {
                        log.error(err, "error saving user for during login");
                        return done(err);
                    }
    ...
    function(req, token, password, done) { // callback with email and password from our form
      ...
      auth.userForResetToken(token, function(err, user) {
        try {
          ...
          if (password != req.param('pass2').trim()) {
